...
async def test_run(hatchet: Hatchet):
    ...
    assert len(successful_runs) == 5, f"Expected 5 successful runs, got {len(successful_runs)}"
    assert len(cancelled_runs) == 1, f"Expected 1 cancelled run, got {len(cancelled_runs)}"
